    ...
    public void authenticate(final Promise promise) {
        ...
        getFingerprintIdentify().startIdentify(MAX_AVAILABLE_TIMES, new FingerprintIdentifyListener() {
            ...
            public void onNotMatch(int availableTimes) {
                if( availableTimes <= 0 ){
                    mReactContext.getJSModule(RCTDeviceEventEmitter.class).emit("FINGERPRINT_SCANNER_AUTHENTICATION", "AuthenticationLockout");
                } else {
            ...
            public void onFailed(boolean isDeviceLocked) {
                if(isDeviceLocked){
                    promise.reject("AuthenticationFailed", "AuthenticationLockout");
                } else {
